...
def groq_chat(
    ...
    # Send the completion request to the Groq model
    chat_completion = client.chat.completions.create(
        messages=messages, model=groq_model
    )

    # Handle TTS queue setup
    TTS_queue = queue.Queue()
    worker_thread = threading.Thread(
        target=process_TTS_queue, args=(TTS_queue,), daemon=True
    )
    ...
    # Split the response into sentences and process them
    response_sentences = split_sentence(response_text)
