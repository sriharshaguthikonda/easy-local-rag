# Proposal (implementation detail): Migrate Chroma ingest/query to Qwen3 Embedding 0.6B

**Status:** Interim implementation slice — authorized by
[ADR 0001](../decisions/0001-qwen3-interim-vs-postgres-target.md).

**Authority:** This is a proposal, not a canonical acceptance contract. GitHub
issues [#17](https://github.com/sriharshaguthikonda/easy-local-rag/issues/17)
and [#20](https://github.com/sriharshaguthikonda/easy-local-rag/issues/20)
remain the canonical tracking surface, and the PostgreSQL/pgvector
consolidation remains the authoritative long-term target. This document may not
relax any canonical scope, invariant, closure gate, approval, or rollback
constraint. It expands
[the preserved proposal summary](qwen3-embedding-migration.md) with executable
implementation detail.

**Why it is being executed anyway:** the Qwen3 store is additive and
env-reversible (it creates a new Chroma path and leaves the mxbai collection
untouched), so it delivers retrieval quality now without pre-empting the
pgvector cutover. See ADR 0001 for the full rationale.
