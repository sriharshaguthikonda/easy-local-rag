...
- **Activation:** after this planning packet merges, branch `codex/issue-015-activate` opens a PR into `main` that owns only this canonical file and `docs/issues/README.md`. It changes this file's `PLANNING` status to `ACTIVE`, roadmap #15 from `planning` to `active`, and roadmap `Active issue` from `none` to `#15`. It records the actual final reviewed packet SHA, immutable docs merge SHA, frozen GUI SHA `619365224f6db3770d3369fd84a295589699e513`, implementation branch `codex/fix-issue-15`, and the three worker-owned files. Only that activation PR's GitHub merge commit SHA activates code work.
